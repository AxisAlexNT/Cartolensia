...
func TestGenerateImageBytesDoesNotWriteCache(t *testing.T) {
	cacheDir := t.TempDir()
	img := image.NewRGBA(image.Rect(0, 0, 32, 16))
	for y := 0; y < 16; y++ {
		for x := 0; x < 32; x++ {
	...
	info, data, err := GenerateImageBytes(context.Background(), asset, bytes.NewReader(buf.Bytes()))
	if err != nil {
		t.Fatal(err)
	}
	if info.Status != StatusReady {
	...
	if len(data) == 0 {
		t.Fatal("expected encoded preview bytes")
	}
	if _, err := os.Stat(filepath.Join(cacheDir, "previews")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("expected no preview cache writes, stat err=%v", err)
	}
}
